...
typically words or phrases, but could also be meta commands.
...
class Translation(object):
    ...
    formatting -- Opaque data holding formatting information stored on the
    translation by the formatter for the purpose of undoing translations.
...
class Translator(object):
    ...
    A Translator takes input via the consume_stroke
...
class _Dictionary(UserDict.DictMixin):
    """A dict that keeps track of the length of its longest key.
    
    This dict is expected to have tuples as the keys. It behaves like a normal
    dict but has an additional attribute named longest_key, which holds the
    ...
    def __setitem__(self, key, value):
        self.longest_key = max(self.longest_key, len(key))
    ...
    def __delitem__(self, key):
        ...
        if len(key) == self.longest_key:
            if self._dict:
                self.longest_key = max(len(x) for x in self._dict.iterkeys())
            else:
                self.longest_key = 0
